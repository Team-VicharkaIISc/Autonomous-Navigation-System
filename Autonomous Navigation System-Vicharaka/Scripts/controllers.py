...
def create_si_position_controller(x_velocity_gain=1, y_velocity_gain=1, velocity_magnitude_limit=0.15):
    ...
    gain = np.diag([x_velocity_gain, y_velocity_gain]) #diagnoally put matrix
    ...
    def si_position_controller(xi, positions):
        ...
        _,N = np.shape(xi) #returns the shape of the matrix
        dxi = np.zeros((2, N)) #creates a matrix of 2XN with all zeros
        ...
        # Threshold magnitude
        norms = np.linalg.norm(dxi, axis=0) #calculates the norm of the matrix Dxi along the axis 0
...
def create_clf_unicycle_position_controller(linear_velocity_gain=0.8, angular_velocity_gain=3):
    ...
    def position_uni_clf_controller(states, positions):
        ...
        poses: 2xN numpy array (of desired positons, [x_goal;y_goal])
        ...
        dxu[1][:]=angular_velocity_gain*(rot_error-states[2][:])

        return dxu

    return position_uni_clf_controller

def create_clf_unicycle_pose_controller(approach_angle_gain=1, desired_angle_gain=2.7, rotation_error_gain=1): #complete reading this part.
    ...
    def pose_uni_clf_controller(states, poses): #states rover ka position, should be recieved from motor encoders. #poses is the goal position
        ...
        for i in range(N_states):
            ...
            alpha = theta - (states[2,i]-poses[2,i]) #error in angle
            alpha = np.arctan2(np.sin(alpha),np.cos(alpha)) #wrap angle between -pi and pi
            ...
            dxu[0,i] = gamma* e* ca
            dxu[1,i] = k*alpha + gamma*((ca*sa)/alpha)*(alpha + h*theta) #dxu[1,i]
...
def create_hybrid_unicycle_pose_controller(linear_velocity_gain=1, angular_velocity_gain=2, velocity_magnitude_limit=0.15, angular_velocity_limit=np.pi, position_error=0.05, position_epsilon=0.02, rotation_error=0.05):
    ...
    si_to_uni_dyn = transformations.create_si_to_uni_dynamics(linear_velocity_gain=linear_velocity_gain, angular_velocity_limit=angular_velocity_limit) #has to be completed
...
states = np.array([[1, 1, 1],[1, 1, 1],[1, 1, 1]])
poses = np.array([[0, 3, 2],[0, 3, 2],[0, 3, 4]])
input_approach_state = np.array([[2, 2],[2, 2],[2, 2]])

#rint(create_hybrid_unicycle_pose_controller(position_error=0, rotation_error= 0, position_epsilon=-1000, velocity_magnitude_limit=0.001)(states=states, poses=poses, input_approach_state=input_approach_state))

if __name__ == "__main__":
    print(create_clf_unicycle_position_controller()(states=np.array([[0],[0] ,[0]]), positions=np.array([[5], [10]])))