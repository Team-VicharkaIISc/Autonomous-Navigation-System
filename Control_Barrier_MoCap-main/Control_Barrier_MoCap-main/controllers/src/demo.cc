...
int main() {
  ...
  auto target_1 = [&ss, &s_eta](const abs_type& idx) {
    ...
    if (0.35 <= (x[0] - s_eta[0] / 2.0) && (x[0] + s_eta[0] / 2.0) <= 0.7 && 
        2.7 <= (x[1] - s_eta[1] / 2.0) && (x[1] + s_eta[1] / 2.0) <= 3.05)
      return true;
    return false;
  };
   /* write target to file */
  write_to_file(ss, target_1, "target_1");
  ...
  auto target_2 = [&ss, &s_eta](const abs_type& idx) {
    ...
    if (2.5 <= (x[0] - s_eta[0] / 2.0) && (x[0] + s_eta[0] / 2.0) <= 2.85 && 
        0.65 <= (x[1] - s_eta[1] / 2.0) && (x[1] + s_eta[1] / 2.0) <= 1.00)
      return true;
    return false;
  };
  /* write target to file */
  write_to_file(ss, target_2, "target_2");

  auto target_3 = [&ss, &s_eta](const abs_type& idx) {
    state_type x;
    ss.itox(idx, x);
    /* function returns 1 if cell associated with x is in target set  */
    if (3.10 <= (x[0] - s_eta[0] / 2.0) && (x[0] + s_eta[0] / 2.0) <= 3.50 && 
        2.45 <= (x[1] - s_eta[1] / 2.0) && (x[1] + s_eta[1] / 2.0) <= 2.85)
      return true;
    return false;
  };
  /* write target to file */
  write_to_file(ss, target_3, "target_3");
  ...
  // ------------------ Synthesis 3 --------------------
  std::cout << "\nSynthesis 3: " << std::endl;
  tt.tic();
  scots::WinningDomain win_3 = scots::solve_reachability_game(tf, target_3);
  tt.toc();
  std::cout << "Winning domain size for tb 3: " << win_3.get_size() << std::endl;

  // ------------------ Writing 1 -------------------- 
  std::cout << "\nWrite controller to control_1.scs \n";
  if(write_to_file(scots::StaticController(ss, is, std::move(win_1)),"control_1"))
    std::cout << "Done. \n";
  
  // ------------------ Writing 2 -------------------- 
  std::cout << "\nWrite controller to control_2.scs \n";
  if(write_to_file(scots::StaticController(ss, is, std::move(win_2)),"control_2"))
    std::cout << "Done. \n";

  // ------------------ Writing 3 -------------------- 
  std::cout << "\nWrite controller to control_3.scs \n";
  if(write_to_file(scots::StaticController(ss, is, std::move(win_3)),"control_3"))
    std::cout << "Done. \n";
